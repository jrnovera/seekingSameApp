...
export default function SettingsScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={[styles.container, { backgroundColor: C.screenBg }]}> 
      ...
      <View style={[styles.headerDecoration, { backgroundColor: C.tint + '10' }]} />
      ...
      <View style={[styles.profileCard, { backgroundColor: C.surface, borderColor: C.surfaceBorder }]}> 
        <View style={styles.profileContent}>
          ...
          <View style={styles.profileInfo}>
            <Text style={[styles.name, { color: C.text }]}>{userDoc?.display_name || user?.displayName || 'User'}</Text>
            <Text style={[styles.email, { color: C.textMuted }]}>{userDoc?.email || user?.email || 'user@example.com'}</Text>
      ...
      <View style={[styles.card, { backgroundColor: C.surface, borderColor: C.surfaceBorder }]}> 
        <RowItem 
          icon={<Ionicons name="notifications-outline" size={20} color={C.tint} />} 
          label="Notifications" 
      ...
      <Text style={[styles.sectionLabel, { color: C.textMuted }]}>Help & Support</Text>
      <View style={[styles.card, { backgroundColor: C.surface, borderColor: C.surfaceBorder }]}> 
        <RowItem 
          icon={<Ionicons name="information-circle-outline" size={20} color={C.tint} />} 
          label="About Us" 
          description="Learn more about our company"
        />
        <Divider color={C.surfaceBorder} />
        <RowItem 
          icon={<Ionicons name="help-circle-outline" size={20} color={C.tint} />} 
          label="Help Center" 
          description="Find answers to common questions"
        />
        <Divider color={C.surfaceBorder} />
        <RowItem 
          icon={<Ionicons name="chatbubble-ellipses-outline" size={20} color={C.tint} />} 
      ...
      <TouchableOpacity 
        style={[styles.logoutCard, { backgroundColor: C.surface, borderColor: C.surfaceBorder }]}
...
function RowItem({ icon, label, description, onPress }: { icon: React.ReactNode; label: string; description?: string; onPress?: () => void }) {
  const scheme = useColorScheme() ?? 'light';
  const C = Colors[scheme];
  return (
    <TouchableOpacity style={styles.row} activeOpacity={0.7} onPress={onPress}>
      <View style={styles.left}>
        ...
        <View style={styles.labelContainer}>
          ...
          {description && <Text style={[styles.rowDescription, { color: C.textMuted }]}>{description}</Text>}
        </View>
      </View>
