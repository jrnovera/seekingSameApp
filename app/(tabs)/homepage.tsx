...
type Property = {
  ...
  location?:
    | string
    | { latitude: string | number; longitude: string | number; type?: string };
...
export default function Homepage() {
  ...
  const getLocationDisplay = (property: Property): string => {
    ...
    const formattedLocation = `${property.state || ""} ${
      property.zipCode || ""
    }`.trim();
        ...
        try {
          ...
          const allUsers = snapshot.docs.map((doc) => {
            ...
            return {
              ...
              ...data,
              name:
                data.displayName || data.name || data.email || "Unknown Host",
              photoURL: data.photoURL || data.avatar,
              rating: data.rating || 4.5,
              listingsCount:
                data.listingsCount || Math.floor(Math.random() * 5) + 1, // Temporary random count
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const items: Property[] = snapshot.docs.map((doc) => {
        ...
        const property = {
          ...
          state:
            data.state ??
            (typeof data.address === "object" ? data.address.state : undefined),
          zipCode:
            data.zipCode ??
            data.zip ??
  ...
  const isSearchActive = () => {
    return (
      searchQuery.trim() !== "" || activeCategory !== null || hasActiveFilters()
    );
  };
  ...
  useEffect(() => {
      ...
      filtered = filtered.filter(
        (property) =>
          ...
          (property.preferences &&
            Array.isArray(property.preferences) &&
            property.preferences.some((preference) =>
              preference.toLowerCase().includes(query)
            ))
      );
    }
    ...
    if (activeCategory) {
      ...
      filtered = filtered.filter((property) => {
        ...
        return (
          ...
          (property.type &&
            property.type.toLowerCase().includes(categoryLower)) ||
          (property.category &&
            property.category.toLowerCase().includes(categoryLower)) ||
          (property.categories &&
            property.categories.toLowerCase().includes(categoryLower)) ||
          // Check in arrays if available
          (property.amenities &&
            Array.isArray(property.amenities) &&
            property.amenities.some((a) =>
              a.toLowerCase().includes(categoryLower)
            )) ||
          // Check if it's an exact match for room type categories
          (categoryLower === "room" &&
            property.type?.toLowerCase() === "room") ||
    ...
    if (activeFilters.city.trim()) {
      filtered = filtered.filter(
        (property) =>
          property.cities
            ?.toLowerCase()
            .includes(activeFilters.city.toLowerCase()) ||
          (typeof property.location === "string" &&
            property.location
              .toLowerCase()
              .includes(activeFilters.city.toLowerCase()))
    ...
    if (activeFilters.state.trim()) {
      filtered = filtered.filter((property) =>
        property.state
          ?.toLowerCase()
          .includes(activeFilters.state.toLowerCase())
      );
    }
    ...
    if (activeFilters.bathroomType) {
      filtered = filtered.filter(
        (property) => property.bathroomType === activeFilters.bathroomType
      );
    }
    ...
    if (activeFilters.capacity !== undefined) {
      ...
      filtered = filtered.filter(
        (property) =>
          property.capacity !== undefined && property.capacity >= minCapacity
      );
    }
    ...
    <>
      ...
      >
        ...
        <View style={styles.headerRow}>
          <Text style={[styles.heyText, { color: C.text }]}>
            Hey,{" "}
            <Text style={{ color: "#3c95a6", fontWeight: "800" }}>
              {userName}
            </Text>
          </Text>
        ...
        {properties.length === 0 && (
          <View style={styles.sampleDataContainer}>
            <Text style={[styles.noDataText, { color: C.textMuted }]}>
              No properties found. Add some sample data to get started:
            </Text>
            <TouchableOpacity
        ...
        >
          ...
          >
            <Ionicons
              name="filter"
              size={18}
              color={hasActiveFilters() ? "#fff" : C.icon}
            />
          </TouchableOpacity>
        </View>

        {/* Category chips in horizontal scrollable row */}
            ...
            {categories.map((c) => (
              <TouchableOpacity
                ...
                style={[
                  ...
                  {
                    backgroundColor:
                      activeCategory === c.name ? C.tint : C.surface,
                    borderColor:
                      activeCategory === c.name ? C.tint : C.surfaceBorder,
                  },
                ]}
                onPress={() => {
        ...
        {isSearchActive() && filteredProperties.length === 0 && (
          <View style={styles.noResultsContainer}>
            ...
            <Text style={[styles.noResultsTitle, { color: C.text }]}>
              No properties found
            </Text>
            ...
            <TouchableOpacity
              style={[
                styles.clearFiltersBtn,
                { borderColor: C.tint, marginTop: 16 },
              ]}
              onPress={clearAllFilters}
            >
              <Text style={[styles.clearFiltersBtnText, { color: C.tint }]}>
                Clear Filters
              </Text>
            </TouchableOpacity>
          </View>
                ...
                {filteredProperties.slice(0, 10).map((item, index) => (
                      ...
                      transform: [
                        { scale: scaleAnim },
                        {
                          translateY: Animated.multiply(
                            slideAnim,
                            new Animated.Value(index * 0.5 + 1)
                          ),
                        },
                      ],
                    }}
                  >
                    <TouchableOpacity
                      ...
                      onPress={() =>
                        router.push(`/property/details?id=${item.id}`)
                      }
                    >
                      <RemoteImage
                      ...
                      <View style={styles.featureMeta}>
                        ...
                        <View style={styles.metaRow}>
                          ...
                          />
                          <Text
                            style={[styles.metaText, { color: C.textMuted }]}
                          >
                        ...
                        <View style={styles.detailsRow}>
                          ...
                          {item.BathRoomCount !== undefined && (
                            <View style={styles.detailItem}>
                              ...
                              >
                                {item.BathRoomCount}{" "}
                                {item.BathRoomCount === 1 ? "bath" : "baths"}
                              </Text>
                            </View>
                          )}
                        ...
                        <View style={styles.priceRow}>
                          <Text
                            style={[styles.rentLabel, { color: C.textMuted }]}
                          >
                            Rent
                          </Text>
                          {item.price !== undefined && (
                            <Text style={[styles.priceText, { color: C.text }]}>
                              <Text
                                style={{ color: C.accent2, fontWeight: "800" }}
                              >
                                $
          ...
          <>
            <View style={styles.sectionHeader}>
              ...
              <TouchableOpacity activeOpacity={0.7}>
                <Text style={[styles.viewAll, { color: C.tint }]}>
                  view all
                </Text>
              </TouchableOpacity>
            </View>
                ...
                renderItem={({ item }) => (
                  ...
                  >
                    {item.photoURL ? (
                      ...
                      <View
                        style={[styles.hostAvatar, { backgroundColor: C.tint }]}
                      >
                    ...
                    <View style={{ flex: 1 }}>
                      <Text
                        style={[styles.hostName, { color: C.text }]}
                        numberOfLines={1}
                      >
                        {item.name}
                      </Text>
                      <View style={styles.hostRatingRow}>
                        ...
                        <Text
                          style={[styles.hostRating, { color: C.textMuted }]}
                        >
                          {item.rating}
                        </Text>
                        <Text
                          style={[styles.hostListings, { color: C.textMuted }]}
                        >
                          · {item.listingsCount} listing
                          {item.listingsCount !== 1 ? "s" : ""}
                        </Text>
                      </View>
                    </View>
                  </TouchableOpacity>
        ...
        <View style={styles.sectionHeader}>
          <Text style={[styles.sectionTitle, { color: C.text }]}>
            {isSearchActive() ? "Search Results" : "Listings"}
          </Text>
          {!isSearchActive() && (
            <TouchableOpacity
              activeOpacity={0.7}
              onPress={() => router.push("/viewlisting")}
            >
        ...
        {isLoading ? (
          ...
          <View style={styles.noResultsContainer}>
            <Text style={{ color: C.textMuted, paddingHorizontal: 16 }}>
              No properties found.
            </Text>
          </View>
        ) : (
              ...
              >
                <RemoteImage uri={l.imageUrl ?? null} style={styles.gridPhoto}>
                  {l.categories && (
                    ...
                    >
                      <Text
                        style={[styles.badgeTextSmall, { color: "#ffffff" }]}
                      >
                        {l.categories}
                      </Text>
                    </View>
                ...
                <Text
                  numberOfLines={1}
                  style={[styles.gridTitle, { color: C.text }]}
                >
                  {String(l.title || "Untitled")}
                </Text>
                <View style={styles.metaRow}>
                  <Ionicons name="location" size={14} color={C.icon} />
                  <Text style={[styles.metaText, { color: C.textMuted }]}>
                    {getLocationDisplay(l)}
                  </Text>
                  <TouchableOpacity
                ...
                <View style={styles.gridDetailsRow}>
                  {l.bedroomCount !== undefined && (
                    <View style={styles.gridDetailItem}>
                      ...
                      />
                      <Text
                        style={[styles.gridDetailText, { color: C.textMuted }]}
                      >
                        {l.bedroomCount}
                      </Text>
                    </View>
                  ...
                  {l.price !== undefined && (
                    <Text style={[styles.gridPrice, { color: C.accent2 }]}>
                      ...
                      {typeof l.price === "number"
                        ? l.price
                        : parseInt(
                            String(l.price).replace(/[^0-9]/g, "") || "0"
                          )}
                    </Text>
                  )}
                </View>
